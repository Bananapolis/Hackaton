...
def get_db() -> Generator[sqlite3.Connection, None, None]:
    """Yield a committed-on-exit, auto-closing SQLite connection."""
...
def init_db() -> None:
    ...
    with get_db() as conn:
        ...
        if "owner_user_id" not in {row["name"] for row in cursor.fetchall()}:
            conn.execute("ALTER TABLE sessions ADD COLUMN owner_user_id INTEGER")

        cursor = conn.execute("PRAGMA table_info(presentations)")
        if "session_code" not in {row["name"] for row in cursor.fetchall()}:
            conn.execute("ALTER TABLE presentations ADD COLUMN session_code TEXT")
...
def set_session_active(code: str, active: bool) -> None:
    with get_db() as conn:
        conn.execute(
            "UPDATE sessions SET active = ? WHERE code = ?",
...
def session_exists(code: str) -> bool:
    with get_db() as conn:
        row = conn.execute(
            "SELECT code FROM sessions WHERE code = ?", (code,)
        ).fetchone()
...
def save_quiz_for_user(
    *,
    ...
    valid_option_ids = {opt.id.upper() for opt in quiz.options}
    if correct_option_id not in valid_option_ids:
        raise ValueError("correct_option_id must match an option id")

    serialized_options = json.dumps([opt.model_dump() for opt in quiz.options])
